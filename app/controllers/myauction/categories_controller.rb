class Myauction::CategoriesController < ApplicationController
  ...
  def create
    if @category.update(category_params)
      redirect_to "/myauction/categories?parent_id=#{@category.parent_id}", notice: "#{@category.name}を登録しました"
    else
      render :new
    end
  end
  ...
  def update
    @category = Category.find(params[:id])
    if @category.update(category_params)
      redirect_to "/myauction/categories?parent_id=#{@category.parent_id}", notice: "#{@category.name}を変更しました"
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to "/myauction/categories?parent_id=#{@category.parent_id}", notice: "#{@category.name}を削除しました"
  end
