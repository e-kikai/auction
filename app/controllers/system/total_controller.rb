class System::TotalController < System::ApplicationController
  ...
  def products
    ...
    where_cr  = ["DATE(created_at) BETWEEN ? AND ?", rstart, rend]
    where_str = ["DATE(dulation_start) BETWEEN ? AND ?", rstart, rend]
    where_end = ["DATE(dulation_end) BETWEEN ? AND ?", rstart, rend]
